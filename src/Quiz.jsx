...
const Quiz = ({ category }) => {
    ...
    useEffect(() => {
        const fetchQuestions = async () => {
            setLoading(true);
            try {
                const res = await fetch(`https://music-quiz-backend.vercel.app/quiz?category=${category}`);
    ...
    // ───────────────────────────────────────────────
    if (loading) return <h2 style={{ marginTop: "2rem", color: "#666", textAlign: "center" }}>Loading questions...</h2>;
    if (error) return <h2 style={{ marginTop: "2rem", color: "#666", textAlign: "center" }}>{error}</h2>;
    if (!questions.length) return <h2 style={{ marginTop: "2rem", color: "#666", textAlign: "center" }}>No questions found.</h2>;
    ...
    if (quizComplete) {
        return (
            <div style={{ padding: "1rem", textAlign: "center" }}>
                ...
                <p>
                    You attempted <strong>{attempted}</strong> out of <strong>{questions.length}</strong> questions.
                </p>
                <p>
